...
fn register_definition(
    ...
    let cfgs = quote![#(#[cfg(#cfgs)])*];
    let element_type = &register.element_type;
    quote! {
        #cfgs pub struct #struct_name<B: Bus>(B);
        #cfgs impl<B: Bus> #struct_name<B> {
            pub unsafe fn new(address: B) -> Self { Self(address) }
        }
        #cfgs impl<B: Bus> #tock_registers::internal::core::clone::Clone for #struct_name<B> {
            fn clone(&self) -> Self { *self }
        }
        #cfgs impl<B: Bus> #tock_registers::internal::core::marker::Copy for #struct_name<B> {}
        #cfgs impl<B: Bus> #tock_registers::Block for #struct_name<B> {
            type Address = B;
        ...
        #cfgs impl<B: Bus> #tock_registers::Register for #struct_name<B> {
            type DataType = #element_type;
        }
        #(#cfgs #operations!(real_impl, #struct_name, #element_type,);)*
    }
}
