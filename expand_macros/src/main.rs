...
// CHECKPOINT: I just realized that this should work differently if pretty printing is off, or if
// any intermediate outputs fail to parse. Instead of erroring, it should just dump the tokens in
// order (so it still works).
...
use syn::{
    ...
    parse_file, Attribute, File, Item,
    Item::Macro,
    Result,
};
